...
module.exports = (robot) ->
  robot.respond /j(?:enkins)? build ([\w\.\-_ ]+)(, (.+))?/i, (msg) ->
    ...
    req.get() (err, res, body) ->
      if err
        ...
        for job in content.jobs
          ...
          if job.name.indexOf("DEV_ENV_INST") != -1
             jobs.dev.push(job.name)
          else
             jobs.prod.push(job.name)
        fs.writeFile(jobFileName, JSON.stringify(jobs), (error) -> throw error if error)
    providedJobName = msg.match[1]
    isDev = false
    if providedJobName.indexOf("DEV_ENV_INST") != -1
       isDev = true
    fs = require 'fs'
    ...
    for user in userListArray
      if user.username == msg.message.user.name
        userHitCount = 1
        if user.isAuth
          msg.send "User is Authenticated"
        else
          msg.send "User has not privilege to execute the build"
          return
    if userHitCount == 0
      msg.send "User has not privilege to execute the build"
      return
    msg.send "Building - #{providedJobName}"
  ...
  robot.respond /j(?:enkins)? help( (.*))?/i, (msg) ->
    ...
    helpText.push("jenkins register <prod/dev> -- Register the user to appropriate environment i.e dev or prod")
    helpText.push("jenkins admin list <prod/dev> --key=<admin's Password> -- List the Users who are belongs to dev or prod environment group")
    helpText.push("jenkins admin approve <prod/dev> --key=<admin's Password> --user=<username> -- Approves the user to have build privileges for the environment provided")
    helpText.push("jenkins admin decline <prod/dev> --key=<admin's Password> --user=<username> -- Decline/Remove user's build privileges for the environment provided")
    helpText.push("jenkins list -- List all the jobs available in the jenkins environment")
    helpText.push("jenkins build <jobName> -- Based on the user's privilege job will be initiated in jenkins console")
    for helpVal in helpText
      resultData += "\n" + helpVal 
    msg.send resultData
